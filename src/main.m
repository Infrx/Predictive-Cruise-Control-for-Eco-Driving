...
%        0.01, 0.001, 0.0001];
kappa = [
    0.01, 0, 0;              % κ0,j terms: keep zero idle consumption
    1, 0.00005, 0.00001;      % κ1,j terms: slightly increased for better high-speed performance
    0.01, 0.0006, 0         % κ2,j terms: reduced overall but added mid-term for better power band
];
% kappa = [
...
figure;
subplot(4, 1, 1);
plot(time(2:end), x.v_h(2:end), time(2:end), px.v_p(2:end),'LineWidth', 2);
xlabel('Time (s)');
ylabel('Speed (m/s)');
...
subplot(4, 1, 2);
plot(time(2:end), x.s_h(2:end), time(2:end), px.s_p(2:end), 'LineWidth', 2);
xlabel('Time (s)');
ylabel('Position (m)');
...
subplot(4, 1, 3);
plot(time(2:end), x.n_g(2:end), 'LineWidth', 2);
xlabel('Time (s)');
